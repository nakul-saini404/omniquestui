...
import type { PersonalityReport, LeadFormData } from "@/lib/personality"; // ✅ correct source
...
const LOADS = [
  "Sending answers to AI…",
  "Analysing 6 personality dimensions…",
  "Identifying your unique type…",
  "Mapping career strengths…",
...
export default function PersonalityTestPage() {
  ...
  // ✅ LeadFormData is imported from @/lib/personality — matches where it is defined
  async function handleFormSubmit(data: LeadFormData) {
    ...
    try {
      const aiRes = await fetch("/api/personality-test", {
        ...
        body:    JSON.stringify({ answers, studentName: data.fullName, leadData: data }),
      });
      ...
      fetch("/api/submit-lead", {
        method:  "POST",
        headers: { "Content-Type": "application/json" },
        body:    JSON.stringify({ leadData: data, answers, report: aiReport }),
      });
    } catch (err) {
      console.error("OpenAI failed, using fallback:", err);
      const { computeReport } = await import("@/lib/personality");
      const fb = computeReport(answers, data.fullName, data);
      setReport(fb);
      fetch("/api/submit-lead", {
        method:  "POST",
        headers: { "Content-Type": "application/json" },
        body:    JSON.stringify({ leadData: data, answers, report: fb }),
      });
    } finally {
      clearInterval(iv);
      setStage("report");
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#060812", color: "#f1f5ff", fontFamily: "var(--font-body)", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0 }}>
        {[
          { w: 600, h: 600, bg: "rgba(91,138,255,.09)",  top: -200,   left: -100, dur: "9s"  },
          { w: 500, h: 500, bg: "rgba(167,139,250,.07)", bottom: -100, right: -80, dur: "11s", delay: "-4s"  },
          { w: 380, h: 380, bg: "rgba(0,201,177,.06)",   top: "40%",  left: "55%", dur: "13s", delay: "-7s" },
        ].map((o, i) => (
          <div
            key={i}
            style={{
              ...
              top:            o.top,
              left:           o.left,
              bottom:         (o as { bottom?: number }).bottom,
              right:          (o as { right?: number }).right,
              animation:      `orbFloat ${o.dur} ease-in-out infinite alternate`,
              animationDelay: (o as { delay?: string }).delay,
            }}
          />
        ))}
      </div>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, padding: "16px 40px", display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(6,8,18,.65)", backdropFilter: "blur(24px)", borderBottom: "1px solid rgba(255,255,255,.07)" }}>
        <Link href="/" style={{ fontFamily: "var(--font-head)", fontSize: "1.2rem", fontWeight: 700, background: "var(--grad-teal)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", textDecoration: "none" }}>
          OmniQuest
        </Link>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          {stage === "quiz" && (
            <span style={{ fontSize: ".8rem", color: "rgba(241,245,255,.4)" }}>Q {current + 1}/{total}</span>
          )}
          <span style={{ fontSize: ".72rem", color: "rgba(241,245,255,.4)", letterSpacing: ".1em", textTransform: "uppercase" }}>
            AI Personality Test
          </span>
        </div>
      </nav>
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 720, margin: "0 auto", padding: "96px 24px 60px" }}>
        ...
        {stage === "intro" && (
          <div style={{ textAlign: "center", animation: "fadeUp .7s ease both" }}>
            ...
            <h1 style={{ fontFamily: "var(--font-head)", fontSize: "clamp(2.2rem,5vw,3.5rem)", fontWeight: 800, lineHeight: 1.14, marginBottom: 18 }}>
              Discover Your<br />
              <span style={{ background: "linear-gradient(135deg,#5b8aff,#a78bfa,#00C9B1)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
                True Potential
              </span>
            </h1>
            ...
            <div style={{ display: "flex", gap: 20, justifyContent: "center", marginBottom: 44, flexWrap: "wrap" }}>
              {[["⏱️", "5 min"], ["⚡", "AI"], ["📊", "Full report"], ["🎯", "Program match"]].map(([ic, lb]) => (
                <div key={lb} style={{ textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: ".74rem", color: "rgba(241,245,255,.4)" }}>{lb}</div>
                </div>
              ))}
            </div>
            <div style={{ display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap" }}>
              <button
                ...
                style={{ padding: "15px 40px", borderRadius: 50, background: "var(--grad-teal)", color: "white", fontSize: "1rem", fontWeight: 700, border: "none", cursor: "pointer", boxShadow: "0 8px 32px rgba(0,201,177,.35)", transition: "transform .2s, box-shadow .2s" }}
              ...
              <Link href="/" style={{ padding: "15px 28px", borderRadius: 50, background: "rgba(255,255,255,.06)", border: "1.5px solid rgba(255,255,255,.15)", color: "rgba(241,245,255,.7)", fontSize: "1rem", textDecoration: "none", display: "inline-flex", alignItems: "center" }}>
                ← Home
              </Link>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, marginTop: 52 }}>
              {Object.entries(ICONS).map(([cat, ic]) => (
                <div
                  key={cat}
                  style={{ padding: 16, borderRadius: 12, background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.07)", textAlign: "center", transition: "border .2s" }}
          ...
          <div style={{ animation: "fadeUp .45s ease both" }}>
            ...
            <div style={{ marginBottom: 32 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                <span style={{ fontSize: ".76rem", color: "rgba(241,245,255,.45)", textTransform: "uppercase", letterSpacing: ".1em" }}>
                  {ICONS[q.category]} {q.category.charAt(0).toUpperCase() + q.category.slice(1)}
                </span>
            ...
            <div style={{ background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 20, padding: "32px 28px", marginBottom: 18 }}>
              <div style={{ fontSize: ".7rem", color: "#5b8aff", fontWeight: 600, letterSpacing: ".12em", textTransform: "uppercase", marginBottom: 14 }}>
                Question {current + 1}
              </div>
              <h2 style={{ fontFamily: "var(--font-head)", fontSize: "clamp(1.1rem,3vw,1.45rem)", fontWeight: 700, lineHeight: 1.4, marginBottom: 26 }}>
                {q.text}
              </h2>
            ...
            <div style={{ display: "flex", gap: 11 }}>
              <button
                onClick={prev}
                disabled={current === 0}
                style={{ padding: "12px 22px", borderRadius: 12, background: "rgba(255,255,255,.06)", border: "1px solid rgba(255,255,255,.1)", color: current === 0 ? "rgba(241,245,255,.2)" : "rgba(241,245,255,.65)", cursor: current === 0 ? "default" : "pointer", fontSize: ".9rem", fontFamily: "var(--font-body)" }}
              >
                ← Back
              </button>
              <button
        ...
        {/* ── LEAD FORM ── */}
        {stage === "form" && <LeadForm onSubmit={handleFormSubmit} mode="post_quiz" />}
        ...
        {stage === "loading" && (
          <div style={{ textAlign: "center", animation: "fadeUp .5s ease both", padding: "40px 0" }}>
            ...
            <h2 style={{ fontFamily: "var(--font-head)", fontSize: "1.6rem", fontWeight: 800, marginBottom: 10 }}>
              AI is analysing your profile
            </h2>
            <p style={{ color: "rgba(241,245,255,.4)", fontSize: ".9rem", marginBottom: 32, minHeight: 26, transition: "opacity .5s" }}>
              {loadMsg}
            </p>
            <div style={{ display: "flex", gap: 8, justifyContent: "center", marginBottom: 40 }}>
              {[0, 1, 2, 3, 4].map(i => (
                <div key={i} style={{ width: 10, height: 10, borderRadius: "50%", background: "var(--teal)", animation: "typingDot 1.4s ease-in-out infinite", animationDelay: `${i * .18}s` }} />
              ))}
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 9, maxWidth: 420, margin: "0 auto" }}>
              {[["🧠","Processing your answers"],["📊","Scoring 6 dimensions"],["⚡","AI reasoning"],["🎯","Matching personality type"],["💼","Finding career paths"],["🎓","Picking best program"]].map(([ic, tx]) => (
                <div key={tx} style={{ padding: "10px 14px", borderRadius: 10, background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.06)", display: "flex", alignItems: "center", gap: 8, fontSize: ".78rem", color: "rgba(241,245,255,.4)" }}>
                  <span>{ic}</span>{tx}
                </div>
              ))}
            </div>
