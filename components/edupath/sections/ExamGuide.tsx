...
export default function ExamGuide({ countries, stream, grade, career }: Props) {
  ...
  const relevantExams = noSelection
    ? EXAM_DB
    : getRelevantExams(countries, stream, career);
  const relevantScholarships = noSelection
    ? SCHOLARSHIPS
    : getRelevantScholarships(countries, stream, career);
  const relevantVisas = noSelection ? VISA_DB : getRelevantVisas(countries);

  const activeExam =
    relevantExams.find((e) => e.name === selectedExam) ?? relevantExams[0];
  const activeVisa =
    relevantVisas.find((v) => v.country === selectedVisa) ?? relevantVisas[0];
            ...
            >
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                ...
                >
                  ...
                  <div
                    style={{ fontSize: 11, color: "var(--ep-muted)", lineHeight: 1.5, marginBottom: 10 }}
                  >
                ...
                {relevantVisas.map((v) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        style={{ fontSize: 11, color: "var(--ep-muted)", marginTop: 2 }}
                      >
                        {v.visaType}
                      </div>
                    </button>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      >
        ...
        >
          <div>
            ...
            <div style={{ fontSize: 13, color: "var(--ep-muted)", marginBottom: 10 }}>
              {exam.fullName}
            </div>
          ...
          <div style={{ textAlign: "right" }}>
            ...
            <div style={{ fontSize: 11, color: "var(--ep-muted)", marginTop: 6 }}>
              Cost: {exam.cost}
            </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        ...
        <InfoBox
          ...
          items={
            exam.optionalFor.length > 0
              ? exam.optionalFor
              : ["Not commonly listed as optional — see country requirements"]
          }
          color="#93c5fd"
      ...
      {exam.notRequiredFor.length > 0 && (
        <InfoBox
          title="❌ Not Required For"
          items={exam.notRequiredFor}
          color="var(--ep-muted)"
          bg="rgba(255,255,255,.03)"
          border="var(--ep-border)"
        />
      ...
      {/* Benefits */}
      <SectionBox
        label="✦ Why This Exam Matters"
        labelColor="var(--ep-accent)"
      >
        <ul style={{ margin: 0, paddingLeft: 18, display: "flex", flexDirection: "column", gap: 6 }}>
          {exam.benefits.map((b, i) => (
            <li key={i} style={{ fontSize: 12, color: "var(--ep-text)", lineHeight: 1.5 }}>
              {b}
            </li>
          ))}
        </ul>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <SectionBox label="📋 How to Apply" labelColor="var(--ep-gold)">
          <div
            style={{ fontSize: 12, color: "var(--ep-text)", lineHeight: 1.6, marginBottom: 8 }}
          >
            {exam.howToApply}
          </div>
          <div style={{ fontSize: 11, color: "var(--ep-muted)", marginBottom: 4 }}>
      ...
      <SectionBox label="📅 Prep Timeline" labelColor="var(--ep-green)">
            ...
            {(["2026", "2027"] as const).map((y) => (
              <button
                ...
                style={{
                  ...
                  cursor: "pointer",
                  background:
                    timelineYear === y ? "rgba(16,185,129,.15)" : "transparent",
                  border: `1px solid ${
                    timelineYear === y
                      ? "rgba(16,185,129,.4)"
                      : "var(--ep-border)"
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
          {(timelineYear === "2026" ? exam.timeline2026 : exam.timeline2027).map(
            (item, i, arr) => (
              <div
                key={i}
                style={{
              ...
              >
                ...
                <div style={{ paddingBottom: 2 }}>
                  ...
                  <div
                    style={{ fontSize: 12, color: "var(--ep-text)", lineHeight: 1.4 }}
                  >
                    {item.action}
                  </div>
                </div>
      ...
      {exam.eduquestNote && (
        ...
        >
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: "#c7d9f5", lineHeight: 1.6 }}>
              {exam.eduquestNote}
            </div>
          </div>
          <a
      ...
      >
        ...
        >
          <div style={{ fontSize: 12, color: "var(--ep-muted)" }}>
            ⏱️ Processing:{" "}
            <span style={{ color: "var(--ep-text)" }}>{visa.processingTime}</span>
          </div>
          <div style={{ fontSize: 12, color: "var(--ep-muted)" }}>
            💰 Cost:{" "}
            <span style={{ color: "var(--ep-text)" }}>{visa.cost}</span>
          </div>
          <div style={{ fontSize: 12, color: "var(--ep-muted)" }}>
            📋 Trigger:{" "}
            <span style={{ color: "var(--ep-text)" }}>{visa.triggerDocument}</span>
          </div>
        </div>
      </div>
        ...
        >
          ...
          >
            {visa.interviewTips.map((tip, i) => (
              <li
                key={i}
                style={{ fontSize: 12, color: "var(--ep-text)", lineHeight: 1.5 }}
              >
                {tip}
              </li>
            ))}
    ...
    >
      ...
      >
        <div style={{ flex: 1 }}>
          ...
          >
            <span
              style={{ fontSize: 12, color: "var(--ep-green)", fontWeight: 600 }}
            >
              {scholarship.amount}
            </span>
